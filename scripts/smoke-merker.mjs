...
// tid, nivå, døgnet-rundt, comeback, PR, kilder og nyeMerker-diffen.
import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, join } from 'node:path';
import { beregnMerker, nyeMerker, MERKER, MERKE_KATEGORIER } from '../js/merker.js';
...
const okt = (n, ekstra = {}) => ({ dato: dag(n), varighetMin: 20, intensitet: 3, xp: 20, bevegelse: 'walk', ...ekstra });
...
const tomt = beregnMerker({ globalXp: 0 }, [], []);
...
// --- Første bevegelse ---
const en = beregnMerker({ globalXp: 20 }, [okt(0)], []);
...
const mStreak = beregnMerker({ globalXp: 0 }, uke, []);
...
sjekk(!finn(beregnMerker({ globalXp: 0 }, hull, []), 'streak-3').oppnadd, 'hull i rekka teller ikke');

// --- Prøv noe nytt: distinkte bevegelsestyper ---
const typer = ['walk', 'run', 'yoga', 'strength'].map((b, i) => okt(i, { bevegelse: b }));
const mTyper = beregnMerker({ globalXp: 0 }, typer, []);
...
sjekk(finn(beregnMerker({ globalXp: 0 }, gammel, []), 'nytt-2').oppnadd, 'gamle modalitets-økter teller som egne typer');

// --- Tid i bevegelse (kumulative minutter) ---
const tid = [okt(2, { varighetMin: 30 }), okt(1, { varighetMin: 40 })];
const mTid = beregnMerker({ globalXp: 0 }, tid, []);
...
// --- Nivå fra profilens XP ---
const mNiva = beregnMerker({ globalXp: 900 }, [okt(0, { xp: 900 })], []);
sjekk(finn(mNiva, 'niva-5').oppnadd, '900 XP er forbi nivå 5');
sjekk(!finn(mNiva, 'niva-10').oppnadd, 'men ikke nivå 10');
...
const mStore = beregnMerker({ globalXp: 0 }, store, []);
...
const mDognet = beregnMerker({ globalXp: 0 }, dognet, []);
...
sjekk(finn(beregnMerker({ globalXp: 0 }, helg, []), 'helg').oppnadd, 'lørdag + søndag samme helg gir Helgekriger');
...
const mRytme = beregnMerker({ ukemaal: 4, globalXp: 0 }, rytme, []);
sjekk(finn(mRytme, 'uker-2').verdi >= 1, 'ukerytme telles');

// --- Comeback: 5+ dagers pause og så tilbake ---
const comeback = [okt(10), okt(1)];
sjekk(finn(beregnMerker({ globalXp: 0 }, comeback, []), 'comeback').oppnadd, 'retur etter pause gir Comeback');
sjekk(!finn(beregnMerker({ globalXp: 0 }, [okt(2), okt(0)], []), 'comeback').oppnadd, 'kort pause er ikke comeback');
sjekk(finn(beregnMerker({ globalXp: 0, harComeback: true }, [okt(0)], []), 'comeback').oppnadd, 'gammelt harComeback-flagg respekteres');

// --- Kilder og planer ---
const kilder = [okt(2, { kilde: 'strava' }), okt(1, { kilde: 'bibliotek' }), okt(0, { test: true })];
const planer = [{ status: 'gjort' }, { status: 'planlagt' }];
const mKilder = beregnMerker({ globalXp: 0 }, kilder, planer);
...
const før = beregnMerker({ globalXp: 0 }, [], []);
const etter = beregnMerker({ globalXp: 20 }, [okt(0)], []);
